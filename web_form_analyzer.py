...
# SKIP_PATTERNS = [
#     "/vulnerabilities/csrf/",          # DVWA Brute-Force page
#     # thêm pattern khác nếu muốn …
# ]
...
#         if pat in base_url:
#             print(f"[+] Skip pattern matched ({pat}); skipping '{base_url}'")
#             return base_url          # không ghi log, không xử lý gì thêm
...
#     import sys
#     test_url = sys.argv[1] if len(sys.argv) > 1 else "http://localhost:8085/vulnerabilities/sqli/"
...
def analyze_and_capture_url(base_url: str) -> str:
    ...
    target_domain = parsed.hostname or "localhost"
    ...
    with sync_playwright() as pw:
        ...
        # 2. Load cookie from header.txt if available
        if os.path.exists(HEADER_FILE):
            raw = open(HEADER_FILE, "r", encoding="utf-8").read().strip()
            if raw.lower().startswith("cookie:"):
                raw = raw.split("Cookie:", 1)[1].strip()
            ...
            if cookies:
                context.add_cookies(cookies)
                print(f"[+] Injected cookies: {[c['name'] for c in cookies]}")

        # 3. Debug injected cookies
        print("[+] Cookies inside browser:")
        for c in context.cookies():
            print(f"    {c['name']} = {c['value']} | domain: {c['domain']}")

        # 4. Try accessing the base URL
        try:
        ...
        # 5. If redirected to login, auto login
        if "/login" in page.url:
            print("[!] Cookie invalid or expired. Logging in with admin/password...")
            login_url = f"{parsed.scheme}://{parsed.netloc}/login.php"
            page.goto(login_url)
            page.fill("input[name='username']", "admin")
            page.fill("input[name='password']", "password")
            page.click("input[type='submit']")
        ...
        # 6. Submit form if found
        form = page.query_selector("form")
        if form:
            for inp in form.query_selector_all("input[name]"):
                typ = (inp.get_attribute("type") or "text").lower()
                name = inp.get_attribute("name")
                if typ in ["text", "search", "email", "url", "number", "password"]:
                    page.fill(f"input[name='{name}']", "1")
                elif typ == "hidden":
                    val = inp.get_attribute("value") or "1"
                    page.evaluate(f"document.querySelector('input[name=\"{name}\"]').value = '{val}'")
...
if __name__ == "__main__":
    # test_url = "http://localhost:8085/vulnerabilities/sqli/"
    test_url = "http://testphp.vulnweb.com/index.php"
    print("[+] Testing:", test_url)
    result = analyze_and_capture_url(test_url)
